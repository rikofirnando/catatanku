...
get_storage_usage_percentage() {
  used=$(df -P "$1" | awk 'NR==2 {print $3}')
  total=$(df -P "$1" | awk 'NR==2 {print $2}')
...
get_storage_usage_before_v2=$(df -P "$target_directory" | awk 'NR==2 {print "Storage Used in Byte(s):", $3}')
...
if [ "$current_percentage_before" -gt "$threshold_percentage" ]; then
  ...
  echo "Storage usage is above ${threshold_percentage}%. Archiving *.log and/or *.txt files..."
  archive_logs "$target_directory"
  echo "The archiving process is completed."
else
  echo "Storage usage percentage: $current_percentage_before%"
  echo "Storage usage is below ${threshold_percentage}%. No archiving needed."
fi

# Get the storage usage after archiving
current_percentage_after=$(get_storage_usage_percentage "$target_directory")
current_percentage_after_v2=$(df -P "$target_directory" | awk 'NR==2 {print "Storage Used in Byte(s):", $3}')
...
echo "$get_storage_usage_before_v2 = $current_percentage_before%"
echo "2. After archiving: "
echo "$current_percentage_after_v2 = $current_percentage_after%"
echo "                       "
