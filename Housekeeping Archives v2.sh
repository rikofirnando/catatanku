...
if [ "$current_percentage_before" -gt "$threshold_percentage" ]; then
  ...
  echo "Storage usage is above or more than or equivalent to ${threshold_percentage}%. Archiving *.log and/or *.txt files..."
  archive_logs "$target_directory"
  echo "The archiving process is completed."
else
  echo "Storage usage percentage: $current_percentage_before%"
  echo "Storage usage is below or less than or equivalent to ${threshold_percentage}%. No archiving needed."
fi
